...
func TestDecryptCBCArrayv3(t *testing.T) {
	type args struct {
		src []byte
		key []byte
	}
	...
	for i := 0; i < 100000; i++ {
		...
		rand.Read(buf)
		rand.Read(Newkey)
		EncryptCBCArrayV3(buf, Newkey, encryptedBuf)
		tests = append(
			tests,
			struct {
				name string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := DecryptCBCArrayV3(tt.args.src, tt.args.key); !bytes.Equal(got, tt.want) {
				t.Errorf("DecryptCBCArrayV2() = %v, want %v", got, tt.want)
			}
		})
	}
